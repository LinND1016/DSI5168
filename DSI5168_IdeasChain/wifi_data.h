...
char ssid[] = "ASUS244f"; //  your network SSID (name) 
char pass[] = "26516869";    // your network password (use for WPA, or use as key for WEP)
...
const String DID = "2787343414636260"; //裝置id
const String SID_sleep = "button"; //可以自己取裝置名稱 sensor id
...
void getdata(String IDEASCHAIN_DID, String IDEASCHAIN_AK, String IDEASCHAIN_SID){  //  資料下載
 client.stop();
 if (client.connect("ideaschain.com.tw", 80)) {
       ...
       //Serial.println(line); 
        ...
        if(str[2] == '{'){//是json array 格式
           ...
           if(!root.success()){
               ...
               const char* value = root[0]["value"][0];
               Serial.print("value=");
